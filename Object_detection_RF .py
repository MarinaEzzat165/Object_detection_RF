...
print(os.listdir("D:/level 3 part 1/AI/PROJECTS/datasets of AI"))
...
for directory_path in glob.glob("D:/level 3 part 1/AI/PROJECTS/datasets of AI/Training/*"):
...
for directory_path in glob.glob("D:/level 3 part 1/AI/PROJECTS/datasets of AI/Testing/*"):
...
DT_model= DecisionTreeClassifier(max_depth=200,random_state = 0) 
# Train the model on training data
# For sklearn no one hot encoding
DT_model.fit(X_for_RF, y_train )
...
#Prediction using DT model (Has Less Accuracy)
...
print ("Accuracy Using Random Forest = ",
       metrics.accuracy_score(test_labels, prediction_RF) *100 ,"%" )
# accuracy for DT 
print ("Accuracy Using Decision Trees = ",
       metrics.accuracy_score(test_labels, prediction_DT) *100,"%")

#Confusion Matrix - verify accuracy of each class
from sklearn.metrics import confusion_matrix
xx = confusion_matrix(test_labels, prediction_RF)
#print(xx)
sns.heatmap(xx , annot=True)


#Check results on a few select images for RF 
...
#Check results on a few select images for DT  
n=np.random.randint(0, x_test.shape[0])
img = x_test[n]
plt.imshow(img)
input_img = np.expand_dims(img, axis=0) #Expand dims so the input is (num images, x, y, c)
input_img_feature=VGG_model.predict(input_img)
input_img_features=input_img_feature.reshape(input_img_feature.shape[0], -1)
prediction_DT = RF_model.predict(input_img_features)[0] 
# Reverse the label encoder to original name
prediction_DT = le.inverse_transform([prediction_RF]) 
print("The prediction for this image is: ", prediction_DT)
print("The actual label for this image is: ", test_labels[n])
...
upload= tk.Button(root, text='Click here to Upload Images',width=30,command = lambda:upload_file() ,font=font2)
